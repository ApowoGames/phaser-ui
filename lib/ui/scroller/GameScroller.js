...
class GameScroller extends BaseUI {
    ...
    resetFromJSON(config) {
        ...
        this.mConfig.space = config.space || 0;
        this.mConfig.mask = config.mask !== undefined ? config.mask : true;
    ...
    */
    ...
    Sort(isFixed = false, iscallValue = true) {
        ...
        const space = this.mConfig.space;
        const padding = this.mConfig.padding;
        ...
        if (this.mConfig.orientation === 1) {
            ...
            let offsety = 0;
            if (this.mConfig.align === 2) offsety = top;
            else if (this.mConfig.align === 0) offsety = -bottom;
            for (const item of activeArr) {
                item.x -= value * 0.5;
                item.y += offsety;
            }
            ...
            let offsetx = 0;
            if (this.mConfig.align === 2) offsetx = left;
            else if (this.mConfig.align === 0) offsetx = -right;
        ...
        let offset1 = this.mConfig.orientation === 1 ? left : top;
        let offset2 = this.mConfig.orientation === 1 ? right : bottom;
        if (value < 0) {
            if (this.mConfig.align === 0) {
                leftBound = -value / 2 - offset2;
                rightBound = -value / 2 - offset2;
            }
            else if (this.mConfig.align === 1) {
                leftBound = rightBound = 0;
            }
            else {
                leftBound = value / 2 + offset1;
                rightBound = value / 2 + offset1;
            }
            bound = leftBound;
        }
        else {
    ...
    clearItems(destroy = true) {
        this.mGameObject.removeAll(destroy);
        ...
        if (inBound) {
            if (this.mCellDownHandler && !this.mMoveing) {
                ...
                for (let i = 0, len = this.mInteractiveList.length; i < len; i++) {
                    ...
                    if (!interactiveObj.visible) continue;
    ...
    checkPointerInBounds(gameObject, pointer, isCell = false) {
        ...
        if (pointer) {
            ...
            const x = (pointer.worldX - worldMatrix.tx) / zoom;
            const y = (pointer.worldY - worldMatrix.ty) / zoom;
