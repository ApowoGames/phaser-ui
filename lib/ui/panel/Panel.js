...
class Panel extends BaseUI {
    ...
    * @param {boolean} [isStatic = false]
     * @protected
     */
    addAtlas(key, texture, data, isStatic = false) {
        if (!this.mResources) {
     ...
     * @protected
     */
    addImage(key, value, isStatic = false) {
        if (!this.mResources) {
            this.mResources = new Map();
        }
        if (value === undefined)
    ...
    loadComplete(loader, totalComplete, totalFailed) {
        ...
        if (totalFailed > 0 && this.mResources && this.mResources.size > 0) {
            // this.reload();
            // 如果加载失败则将面板打开状态调整为false并移除监听，防止由于加载资源失败导致面板不能再打开
            this.mShow = false;
            this.offLoad();
            this.emit("panel_load_fail", this);
            return;
        }
        // this.init();
    }
