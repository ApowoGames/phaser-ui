...
class NinePatch extends RenderTexture {
    constructor(scene, x, y, width, height, key, baseFrame, columns, rows, config) {
        x = x || 0; // GetValue(config, 'x', 0);
        y = y || 0; // GetValue(config, 'y', 0);
        width = width || 1;// GetValue(config, 'width', 1);
        height = height || 1; // GetValue(config, 'height', 1);
        key = key || undefined; // GetValue(config, 'key', undefined);
        baseFrame = baseFrame || undefined;// GetValue(config, 'baseFrame', undefined);
        columns = columns || undefined; // GetValue(config, 'columns', undefined);
        rows = rows || undefined;// GetValue(config, 'rows', undefined);
        // if (IsPlainObject(x)) {
        //     config = x;
        ...
        this.setTexture(key, baseFrame, this.columns, this.rows); // Also update render texture
    }
