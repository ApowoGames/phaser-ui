...
class Table {
    ...
    rowIndexToHeight(start, end) {
        ...
        sum += this.getRowHeight(start) * this.cellOriginY;
        return sum;
    }
    ...
    get totalRowsHeight() {
        if (this._totalRowsHeight === null) {
            ...
            if (this.defaultCellHeightMode) {
                this._totalRowsHeight = this.rowCount * this.defaultCellHeight + this.padY;
            } else {
                var h, sum = 0;
    ...
    get totalColumnWidth() {
        return this.colCount * this.defaultCellWidth + this.padX;
    }
